...
def convert_bert_to_longformer():
    ...
    parser.add_argument('--bert_checkpoint', default='nlpaueb/legal-bert-small-uncased')
    config = parser.parse_args()

    # load pre-trained bert model and tokenizer
    bert_model = AutoModelForMaskedLM.from_pretrained(config.bert_checkpoint)
    tokenizer = AutoTokenizer.from_pretrained(config.bert_checkpoint, model_max_length=4096)
    ...
    lf_config.attention_window = [config.window_size] * bert_config.num_hidden_layers
    ...
    # save model
    lf_model.save_pretrained(f'models/legal-longformer')

    # save tokenizer
    tokenizer.save_pretrained(f'models/legal-longformer')

    # re-load model
    lf_model = AutoModelForMaskedLM.from_pretrained(f'models/legal-longformer')
    batch = tokenizer(['this is a dog', 'this is a cat'], return_tensors='pt')
    lf_model(input_ids=batch['input_ids'], attention_mask=batch['attention_mask'])
