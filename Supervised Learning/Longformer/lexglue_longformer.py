...
def torch_ecthr_data_collator(features: List) -> Dict[str, Any]:
    import torch
    ...
    for k, v in first.items():
        if k != "labels" and v is not None and not isinstance(v, str):
            ...
            batch[k] = torch.tensor([f[k] for f in features]).float()
...
def main():
    ...
    if training_args.do_train:
        train_dataset = load_dataset("lex_glue", name=data_args.task, split="train",
                                     cache_dir=model_args.cache_dir)

    if training_args.do_eval:
        eval_dataset = load_dataset("lex_glue", name=data_args.task, split="validation",
                                    cache_dir=model_args.cache_dir)

    if training_args.do_predict:
        predict_dataset = load_dataset("lex_glue", name=data_args.task, split="test",
                                       cache_dir=model_args.cache_dir)
    ...
    def preprocess_function(examples):
        ...
        global_attention_mask = np.zeros((len(cases), data_args.max_seq_length), dtype=np.int32)
    ...
    if data_args.pad_to_max_length:
        data_collator = torch_ecthr_data_collator
