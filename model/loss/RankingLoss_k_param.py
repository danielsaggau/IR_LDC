...
class AddProjection(nn.Module):
   def __init__(self, model: SentenceTransformer, k=10,mlp_dim=512,embedding_size=512): #removed sentence_embedding_dimension
...
class BregmanRankingLoss(nn.Module):
  ...
  '''
  def __init__(self, model: SentenceTransformer, sigma, temperature, subnets, batch_size, lambda1, lambda2 ,feat_dim=512, subnets=10, scale: float = 20.0, similarity_fct = cos_sim):
        """
        ...
        self.model = model
        self.projection = AddProjection(self,model, mlp_dim=feat_dim, k=subnets)
        self.sigma = sigma
