...
def convert_bert_to_longformer():
    ...
    # save model
    lf_model.save_pretrained(f'models/bio-longformer')

    # save tokenizer
    tokenizer.save_pretrained(f'models/bio-longformer')

    # re-load model
    lf_model = AutoModelForMaskedLM.from_pretrained(f'models/bio-longformer')
    batch = tokenizer(['this is a dog', 'this is a cat'], return_tensors='pt')
    lf_model(input_ids=batch['input_ids'], attention_mask=batch['attention_mask'])
